...
export class RAGService {
  ...
  async ingestMoviesData() {
    return this.indexingService.indexMovieData();
  }

  async retriveReleventDocuments(
    query: string,
    limit: number = 5,
    sourceType?: string
  ) {
    try {
      ...
      const results = await prisma.$queryRaw(Prisma.sql`
        SELECT id, "chunkKey", "sourceType", "sourceId", "sourceLabel", content, metadata, embedding, "isDeleted",
        "deletedAt",  "createdAt", "updatedAt", 1 - (embedding <=> CAST(${vectorLiteral} AS vector)) AS similarity
  ...
  async generateAnswer(
    ...
    try {
      const relevantDocs = await this.retriveReleventDocuments(
        query,
        limit,
        sourceType
      );

      // extract content documents for context
  ...
  async getStats() {
    try {
      ...
      const sourceTypeCounts = await prisma.$queryRaw(Prisma.sql`
            SELECT "sourceType", COUNT(*) as count FROM "document_embeddings" WHERE "isDeleted" = false
        GROUP By "sourceType"            
            `);
