...
> {
  ...
  search(): this {
    const { search, searchTerm } = this.queryParams;
    const searchValue = searchTerm || search;
    const { searchableFields } = this.config;
    // doctorSearchableFields = ['user.name', 'user.email', 'specialities.specialty.title' , 'specialities.specialty.description']
    if (searchValue && searchableFields && searchableFields.length > 0) {
